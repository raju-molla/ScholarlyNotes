...
export default function PaperForm({ paperId, initial }) {
  ...
  stateRef.current = { key, type, authors, title, source, year, volume, issue, pages, publisher, doi, url, fileUrl, abstract, summary, tagsInput, status, myNotes };
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [key, type, authors, title, source, year, volume, issue, pages, publisher, doi, url, fileUrl, abstract, summary, tagsInput, status, myNotes, paperId]);
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-lg border border-accent/20 bg-accent/5 p-4">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <span className="text-sm font-medium text-ink/80">AI summary</span>
            {paperId && (
              <button
                type="button"
                onClick={regenerateSummary}
                disabled={summarizing || !abstract.trim()}
                className="text-xs rounded-md border border-accent/40 text-accent px-2.5 py-1 hover:bg-accent/10 disabled:opacity-50"
              >
                {summarizing ? "Generating…" : summary ? "Regenerate" : "Generate"}
              </button>
            )}
          </div>
          {summaryError && <p className="text-xs text-red-600 mt-2">{summaryError}</p>}
          <p className="text-sm text-ink/70 mt-2">
            {summary || (paperId ? "No summary yet — click Generate to create a plain-language TL;DR from the abstract." : "Generated automatically once you save, if an abstract is present.")}
          </p>
        </div>

        {paperId && initial?.openalexId && <RelatedPapers openalexId={initial.openalexId} />}
