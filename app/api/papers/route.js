...
// Best-effort 2-3 sentence plain-language TL;DR from the abstract. Never
// blocks paper creation — if there's no AI provider configured, or the
// call fails, we just save the paper without a summary.
async function summarize(abstract) {
  if (!abstract || !abstract.trim() || !getAIProvider()) return "";
  try {
...
export async function POST(req) {
  ...
  const summary = body.summary !== undefined ? body.summary : await summarize(body.abstract);
